...
// Hàm gọi API
function fetchData() {
    const SupervisorID = document.getElementById("SupervisorID_search").value || '';  // Lấy mã nhân viên từ ô tìm kiếm
    const url = `supervisors?page=${currentPage}&limit=${limit}&SupervisorID=${SupervisorID}`;  // Tạo URL với các tham số
...
document.getElementById('addSuperviosButton').addEventListener('click', function () {

    // Lấy giá trị từ các ô input
    const SuptId = document.getElementById('SuptId').value;
    const fullName = document.getElementById('fullName').value;
    const email = document.getElementById('email').value;

    // Kiểm tra dữ liệu hợp lệ (nếu cần)
    if (!SuptId || !fullName || !email) {
        iziToast.warning({
            title: 'Cảnh báo',
    ...
    fetch('supervisor/insert', {
        ...
        .then(response => {
            if (!response.ok) {
                throw new Error('Có lỗi xảy ra khi thêm quản đốc.');
            }
        ...
        .then(data => {
            ...
            iziToast.success({
                title: 'Thông báo',
                message: 'Thêm quản đốc thành công!',
                position: 'topRight'
            });
        ...
        .catch(error => {
            ...
            iziToast.error({
                title: 'Lỗi',
                message: 'Không thể thêm quản đốc !',
                position: 'topRight'
            });
        });
});
...
function showModal(userId) {
    ...
    $.ajax({
        url: `get-data-supervisor/${userId}`,  // Thay bằng URL API thực tế của bạn
        method: 'GET',
        success: function (response) {
            if (response.success && response.data) {
                ...
                $('#teamLeaderIdview').val(user.SupervisorID);
...
$('#saveChanges').on('click', function () {
    ...
    $.ajax({
        url: `supervisor/update`,  // Cập nhật URL API của bạn
