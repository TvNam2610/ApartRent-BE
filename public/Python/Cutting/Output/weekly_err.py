...
if __name__=="__main__":
    ...
    this_wk = datetime.today().strftime('%W')
    ...
    date_full_to = (weeknum_full_dates(year,this_wk))[0]
    ...
    sql1 = ('select distinct substring(NEW_FILE,1,5) as "Line", date_format(NEW_TimeUpdate,"%d-%m-%Y") as "Date", '
        ...
        + 'from cutting_system.bundleticket_alert ba where date_format(ba.NEW_TimeUpdate, "%Y%m%d") between  "'+date_from+'" and "'+date_to+'" and ba.NEW_EMPLOYEE <> ba.OLD_EMPLOYEE and `STATUS` = "N" '
    ...
    sql2 = ('SELECT left(MAX(Temp4.FIle),5) as "Line", Temp4.ISSUE_FILE, LEFT(Temp4.TICKET, 6) AS "BUNDLE", max(QC) as "QC", COUNT(Temp4.TICKET) AS "ISSUE", COUNT(EMPLOYEE) AS "SCAN", COUNT(deactive.TICKET) AS "IASCAN", COUNT(Temp4.TICKET)-COUNT(EMPLOYEE)-COUNT(deactive.TICKET) AS "IS_FULL",COUNT(distinct Temp4.MODIFIED) as "MODIFIED", MAX(Temp4.FILE) as "FILE", Temp4.TimeUpdate, TimeModified FROM '
        ...
        + 'INNER JOIN(SELECT TICKET FROM cutting_system.employee_scanticket where TimeUpdate between "'+date_full_from+" 08:00:00"'" and "'+date_full_to+" 07:59:59"'" ) AS Temp1 ON active.TICKET=Temp1.TICKET) AS Temp2 ON active2.`FILE`=Temp2.FILE WHERE active2.FILE != "0") AS Temp3 ON Temp3.TICKET=scan.TICKET) AS Temp4  LEFT JOIN cutting_system.bundleticket_deactive deactive ON Temp4.TICKET=deactive.TICKET '
        + 'GROUP BY Temp4.ISSUE_FILE ORDER BY Line')
    imageFile = pd.read_sql(sql2, engine)
    
    sql3 = ('select * from cutting_system.bundleticket_error be where `Date` between "'+date_from+'" and "'+date_to+'" AND MODIFIED is null')
    ...
    # excel2img.export_img(FILE,bundle_link+ '\\'+week+"_0.png","Sum_bundle",None)
    # excel2img.export_img(FILE,bundle_link+ '\\'+week+"_2.png","Error Page",None)
    # excel2img.export_img(FILE,bundle_link+ '\\'+week+"_3.png","Mix_bundle",None)
    print('finished!')
