...
def camera1Process(image):
    ...
    # scale=25
    # width=int(blur.shape[1]*scale/100)
    # height=int(blur.shape[0]*scale/100)
    # dim=(width, height)
    # blur2=cv2.resize(blur, dim)
    ...
    for cnt in contours:  
        ...
        if w+h>=300 and w+h<1200:
            ...
            if len(barcodes)>0:
                ...
                if len(barcode_temp)==10 and not barcode_temp.isdigit() and barcode_temp.isalnum() and (barcode_temp[6:10]).isdigit():#BUNDLE BARCODE
                ...
                elif barcode_temp[0:2]=='96' and barcode_temp.isdigit() and len(barcode_temp)==10:#PAPER TYPE: SEWING/NSEWING BARCODE
    ...
    for barcode in barcodes:
        ...
        if 'SEW' in barcode_str:
            if paper_type=='':
                ...
                cv2.rectangle(image, (x,y), (x+w,y+h), (255, 0, 0), 4)
        elif barcode_str[0:2]=='96' and barcode_str.isdigit() and len(barcode_str)==10:
            if worklot=='':
                worklot=barcode_str
            ...
            if zbarCheck==False:
                ...
                cv2.rectangle(image, (x,y), (x+w,y+h), (255, 0, 0), 4)
                cv2.circle(image, (x, y+int(h/2)), 5, (0, 128, 255), 10)
        elif len(barcode_str)==10 and not barcode_str.isdigit() and barcode_str.isalnum() and (barcode_str[6:10]).isdigit():
            zbarCheck=False#outter
