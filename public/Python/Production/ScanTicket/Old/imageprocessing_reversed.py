    ...
    for cnt in contours:  
        ...
        if w+h>=100 and w+h<1000:
            ...
            if len(barcodes)>0:
                ...
                if len(barcode_temp)==10 and not barcode_temp.isdigit() and barcode_temp[0]=='K' and barcode_temp.isalnum() and (barcode_temp[6:10]).isdigit():#BUNDLE BARCODE
                ...
                elif barcode_temp[0:2]=='98' and barcode_temp.isdigit() and len(barcode_temp)==10:#PAPER TYPE: SEWING/NSEWING BARCODE
    ...
    for barcode in barcodes:
        ...
        if 'SEW' in barcode_str:
            if paper_type=='':
                ...
                cv2.rectangle(image, (x,y), (x+w,y+h), (255, 0, 0), 4)
        elif barcode_str[0:2]=='98' and barcode_str.isdigit() and len(barcode_str)==10:
            if worklot=='':
                worklot=barcode_str
            ...
            if zbarCheck==False:
                ...
                cv2.rectangle(image, (x,y), (x+w,y+h), (255, 0, 0), 4)
                cv2.circle(image, (x, y+int(h/2)), 5, (0, 128, 255), 10)
        elif len(barcode_str)==10 and not barcode_str.isdigit() and barcode_str[0]=='K' and barcode_str.isalnum() and (barcode_str[6:10]).isdigit():
            zbarCheck=False#outter
    ...
    if len(ID_list)==1 or (len(ID_list)==2 and ID_list[0]['barcode']==ID_list[1]['barcode'] and ID_list[0]['y']+ID_list[0]['w']*0.5>ID_list[1]['y']):
        if ID_list[0]['y']<image.shape[0]/2:
            ...
            if ID_list[0]['y']+ID_list[0]['w']/3<minY:#employee earn all
                for bundle in bundle_list:
                    barcode_dict.append({'ID': ID_list[0]['barcode'], 'bundle': bundle['barcode'], 'IRR': '000', 'worklot': worklot})
                    cv2.line(image, (ID_list[0]['x'], ID_list[0]['y']), (bundle['x'], bundle['y']), (255,0,0), 4)
            else:
                barcode_dict.append({'ID': ID_list[0]['barcode'], 'bundle': barcode_min, 'IRR': '000', 'worklot': worklot})#bundle_list[0]['barcode']
            ...
            if ID_list[0]['y']-ID_list[0]['w']/3>maxY:#employee earn all
                ...
                barcode_dict.append({'ID': ID_list[0]['barcode'], 'bundle': barcode_max, 'IRR': '000', 'worklot': worklot})#bundle_list[0]['barcode']
...
def uploadNonSewingData(QC_ID, barcode_dict, image_name, check, worklot):
    ...
    for barcode in barcode_dict:
        ...
        date=get_date_format(today)
        irr   =barcode['IRR']
        # if QC_ID=='888888' or QC_ID=='777777':
        #     qc    = QC_ID
        # else:
        qc    = '000000'
        thisTime=datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ...
        if len(duplicate_check)>0:
            if str(duplicate_check.iloc[0,0])!=employee:
                ...
                myCursor.execute(query, values)
                mydb.commit()
            query=("update employee_scanticket set IRR='"+irr+"', QC='"+qc+"', FILE='"+ image_name+"', IS_FULL='"+check+"', TimeModified='"+thisTime+"' where TICKET='"+ticket+"';")
            myCursor.execute(query)
            mydb.commit()
        else:
            query=('replace into employee_scanticket (TICKET, EMPLOYEE, DATE, BUNDLE, OPERATION_CODE, IRR, QC, FILE, IS_FULL, TimeUpdate, WORK_LOT) '
                'values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)')
            values=(ticket, employee, date, bundle, code, irr, qc, image_name, check, thisTime, worklot)
            myCursor.execute(query, values)
            mydb.commit()
    mydb.close()
    engine.dispose()
...
def uploadBundleList(bundle_list, nameFile):
    ...
    for bundle in bundle_list:
        bd=bundle['barcode']
        query=('Update bundleticket_active set FILE="'+nameFile+'" where TICKET = "'+bd+'";')
        myCursor.execute(query)
        mydb.commit()
    mydb.close()
...
def checkticket(barcode_dict):
    result=True
    for bundle in barcode_dict:
        check=pd.read_sql('select TICKET from employee_scanticket where TICKET="'+bundle['bundle']+'";', engine)
        engine.dispose()
        if len(check)==0:
            result=False
            break
                        ...
                        if 'done' not in entry:
                            ...
                            if paper_type=='SEWING':#sewing
                                ...
                                try:                                        
                                    ...
                                    print('cant delete file')
                                    continue 
                            # uploadBundleList(bundle_list, nameFile)
                                              
        except:
            err=1
            print(' ok sau khi bo try : something went wrong Reversed')
            continue
        time.sleep(1)
        H=datetime.now().hour
