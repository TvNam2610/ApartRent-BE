    ...
    for cnt in contours:  
        ...
        if w+h>=300 and w+h<1400:
            ...
            if len(barcodes)>0:
                ...
                if (len(barcode_temp)==10 or len(barcode_temp)==8) and not barcode_temp.isdigit() and barcode_temp.isalnum() and (barcode_temp[6:10]).isdigit():#BUNDLE BARCODE
                ...
                elif barcode_temp[0:2]=='92' or barcode_temp[0:2]=='95' and barcode_temp.isdigit() and (len(barcode_temp)==10 or len(barcode_temp)==8):#PAPER TYPE: SEWING/NSEWING BARCODE
    ...
    for barcode in barcodes:
        ...
        if 'SEW' in barcode_str:
            if paper_type=='':
                ...
                cv2.rectangle(image, (x,y), (x+w,y+h), (255, 0, 0), 4)
        elif barcode_str[0:2]=='92' or barcode_str[0:2]=='95'and barcode_str.isdigit() and (len(barcode_str)==10 or len(barcode_str)==8):
            if worklot=='':
                worklot=barcode_str
            ...
            if zbarCheck==False:
                ...
                cv2.rectangle(image, (x,y), (x+w,y+h), (255, 0, 0), 4)
                cv2.circle(image, (x, y+int(h/2)), 5, (0, 128, 255), 10)
        elif (len(barcode_str)==10 or len(barcode_str)==8) and not barcode_str.isdigit() and barcode_str.isalnum() and (barcode_str[6:10]).isdigit():
            zbarCheck=False#outter
...
def irr_upload(ticket, irr, image_name):
    #check IRR
    irr_check=pd.read_sql('select IRR from qc_endline_record where Ticket="'+ticket+'" and IRR = "'+irr+'";', engine)
    engine.dispose()
...
def uploadSewingData(QC_ID, barcode_dict, image_name, check, worklot):
    # Kết nối cơ sở dữ liệu và khởi tạo con trỏ
    ...
    for barcode in barcode_dict:
        ...
        # Kiểm tra trùng lặp
        myCursor.execute(
            'SELECT EMPLOYEE, WORK_LOT, IRR, FILE, TimeUpdate FROM employee_scanticket WHERE TICKET=%s;',
            (ticket,)
        )
        ...
        if duplicate_check:
            ...
            if duplicate_check[0] != employee:
                query = '''
                    REPLACE INTO bundleticket_alert
                    (TICKET, WORK_LOT, OLD_EMPLOYEE, OLD_FILE, OLD_IRR, OLD_TimeUpdate,
                     NEW_EMPLOYEE, NEW_FILE, NEW_IRR, NEW_TimeUpdate, ERR_CODE)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                '''
                values = (
                    ticket, duplicate_check[1], duplicate_check[0], duplicate_check[3], 
                    duplicate_check[2], duplicate_check[4], employee, image_name, irr, thisTime, '1'
                )
                myCursor.execute(query, values)
                mydb.commit()
            ...
            if duplicate_check[2] != irr:
                ...
                values = (
                    ticket, duplicate_check[1], duplicate_check[0], duplicate_check[3], 
                    duplicate_check[2], duplicate_check[4], employee, image_name, irr, thisTime, '2'
                )
                myCursor.execute(query, values)
                mydb.commit()

                # Cập nhật IRR cho `employee_scanticket`
                myCursor.execute(
                    'UPDATE employee_scanticket SET IRR=%s WHERE TICKET=%s;',
                    (irr, ticket)
                )
                mydb.commit()
            ...
            myCursor.execute(
                '''
                UPDATE employee_scanticket
                SET IRR=%s, IRR1=%s, IRR2=%s, QC=%s, FILE=%s, IS_FULL=%s, TimeModified=%s
                WHERE TICKET=%s;
                ''',
                (irr, irr1, irr2, qc, image_name, check, thisTime, ticket)
            )
            mydb.commit()

        else:
            # Chèn mới nếu không có trùng lặp
            query = '''
                REPLACE INTO employee_scanticket
                (TICKET, EMPLOYEE, DATE, BUNDLE, OPERATION_CODE, IRR, IRR1, IRR2, QC, FILE, IS_FULL, TimeUpdate, WORK_LOT)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            '''
            values = (
                ticket, employee, date, bundle, code, irr, irr1, irr2, qc, image_name,
                check, thisTime, worklot
            )
            myCursor.execute(query, values)
            mydb.commit()
...
def uploadNonSewingData(QC_ID, barcode_dict, image_name, check, worklot):
    ...
    # Thiết lập QC mặc định nếu không có giá trị
    qc = QC_ID if QC_ID else '000000'
    ...
    for barcode in barcode_dict:
        ...
        if duplicate_check:
            # Nếu nhân viên khác
            if str(duplicate_check[0]) != employee:
                query = '''
                    REPLACE INTO bundleticket_alert
                    (TICKET, WORK_LOT, OLD_EMPLOYEE, OLD_FILE, OLD_IRR, OLD_TimeUpdate,
                     NEW_EMPLOYEE, NEW_FILE, NEW_IRR, NEW_TimeUpdate, ERR_CODE)
            ...
            myCursor.execute(
                '''
                UPDATE employee_scanticket
                SET IRR=%s, QC=%s, FILE=%s, IS_FULL=%s, TimeModified=%s
                WHERE TICKET=%s;
                ''',
                (irr, qc, image_name, check, thisTime, ticket)
            )
            mydb.commit()
            ...
            query = '''
                REPLACE INTO employee_scanticket
                (TICKET, EMPLOYEE, DATE, BUNDLE, OPERATION_CODE, IRR, QC, FILE, IS_FULL, TimeUpdate, WORK_LOT)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            '''
            values = (
                ticket, employee, date, bundle, code, irr, qc, image_name,
                check, thisTime, worklot
            )
            myCursor.execute(query, values)
            mydb.commit()
...
def uploadBundleList(bundle_list, nameFile):
    ...
    mydb.commit()  # Commit một lần sau khi cập nhật toàn bộ bản ghi
    mydb.close()
...
def uploadErrorData(nameFile):
    ...
    # Chèn bản ghi lỗi vào cơ sở dữ liệu
...
def checkticket(barcode_dict):
    # Sử dụng một kết nối duy nhất cho tất cả các kiểm tra
    mydb = mysql.connector.connect(
        host=hostname, user='i_admin', passwd='Hy$2020', database="pr2k")
    myCursor = mydb.cursor()
    ...
    result = True
    for barcode in barcode_dict:
        # Truy vấn trực tiếp thay vì sử dụng Pandas
        myCursor.execute(
            'SELECT TICKET FROM employee_scanticket WHERE TICKET = %s', (barcode['bundle'],)
        )
...
def process_single_file(filePath):
    ...
    s_barcode2=[]
    try:
        fileLink = os.path.dirname(filePath)
        nameFile = Path(filePath).stem
        doneFile = os.path.join(fileLink, f"{nameFile}_done.jpg")
        
        # Đọc ảnh và xử lý
        image = cv2.imread(filePath)
        output_img = image  # Khởi tạo output_img để tránh lỗi khi không có loại giấy hợp lệ
        ...
        if paper_type=='SEWING':#sewing
            ...
            check = '1' if len(ID_list) == len(barcode_dict) and len(bundle_list) == len(barcode_dict) else '0'
            ...
            print('Process image non paper type')
            image_1,s_barcode1=processSewing(worklot, bundle_list, ID_list, IRR_list, output_img1, nameFile)
            ...
            if len(s_barcode1)>0:
                ...
                if os.path.isfile(doneFile):
                    nameFile += 'f'
                try:
                    print(f"delete file lỗi {doneFile}")
                    cv2.imwrite(fileLink+'\\'+nameFile+'_done.jpg', output_img)
                    os.remove(filePath)
                except:
                    print('cant delete file no paper type')
        else:
            uploadErrorData(nameFile)
            width = int(output_img.shape[1] * scale / 100)
            height = int(output_img.shape[0] * scale / 100)
        ...
        try:
            if checkticket(barcode_dict)==True:
                print('okf2')
                cv2.imwrite(os.path.join(fileLink, f"{nameFile}_done.jpg"), output_img)
                os.remove(filePath)
                print(f"Processed and saved: {doneFile}")
            else:
                print('f2:'+filePath)
        except:
...
if __name__=="__main__":
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        while True:
            try:
                entries = [os.path.join(loadLink, entry) for entry in os.listdir(loadLink) if entry.endswith(".jpg") and "done" not in entry]
                
                futures = {executor.submit(process_single_file, entry): entry for entry in entries}
                for future in as_completed(futures):
                    filePath = futures[future]
                    try:
                        print(f"Read image : {filePath}")
                        result = future.result()
                        if result:
                            print(f"Success process : {filePath}")
                        else:
