...
if __name__=="__main__":
    ...
    sql = ('select distinct substring(NEW_FILE,1,5) as "Line", date_format(NEW_TimeUpdate,"%d-%m-%Y") as "Date", '
        ...
        + 'from pr2k.bundleticket_alert ba where date_format(ba.NEW_TimeUpdate, "%Y%m%d") = "'+date + '" and ba.NEW_EMPLOYEE <> ba.OLD_EMPLOYEE group by substr(NEW_FILE,1,5) ')
    ...
    sql = ('SELECT left(MAX(Temp4.FIle),5) as "Line", Temp4.ISSUE_FILE, LEFT(Temp4.TICKET, 6) AS "BUNDLE", max(QC) as "QC", COUNT(Temp4.TICKET) AS "ISSUE", COUNT(EMPLOYEE) AS "SCAN", COUNT(deactive.TICKET) AS "IASCAN", COUNT(Temp4.TICKET)-COUNT(EMPLOYEE)-COUNT(deactive.TICKET) AS "IS_FULL",COUNT(distinct Temp4.MODIFIED) as "MODIFIED", MAX(Temp4.FILE) as "FILE", Temp4.TimeUpdate, TimeModified FROM '
        ...
        + 'INNER JOIN(SELECT TICKET FROM pr2k.employee_scanticket where `Date` = "'+date +'") AS Temp1 ON active.TICKET=Temp1.TICKET) AS Temp2 ON active2.`FILE`=Temp2.FILE ) AS Temp3 ON Temp3.TICKET=scan.TICKET) AS Temp4  LEFT JOIN pr2k.bundleticket_deactive deactive ON Temp4.TICKET=deactive.TICKET '
        + 'GROUP BY Temp4.ISSUE_FILE ORDER BY Line')
    imageFile = pd.read_sql(sql, engine)
    sql = ('select * from pr2k.bundleticket_error be where be.`DATE` = "'+date + '" and isnull(be.MODIFIED) ')
    image_err = pd.read_sql(sql,engine)
    print(imageFile)
    row = 0
    ...
    image_err.to_excel(writer,sheet_name='image_err', index=False)
    ...
    writer.sheets['image_err'].set_column(xlsxwriter.utility.xl_range(
        0, 0, len(image_err), len(image_err.columns)-1), 15)
    writer.close()
    engine.dispose()
