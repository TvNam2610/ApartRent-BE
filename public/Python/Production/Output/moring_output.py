...
try:
    mydb = mysql.connector.connect(
        host='10.144.12.93', user='i_admin', passwd='Hy$2020', database="pr2k")
...
def main():
    ...
    print(data)
    with pd.ExcelWriter(link+thisWeek+'\\'+date+"_morning.xlsx",mode='w') as writer:
...
def loadimage():
    ...
    FILE = link+thisWeek+'\\'+date+"_morning"
    ...
    for i, ws in enumerate(workbook.Sheets):
        pd_xl_file = pd.ExcelFile(link+thisWeek+'\\'+date+"_morning.xlsx")
        df = pd_xl_file.parse(sheet_name="Sheet1")
        CELLs=df.shape
        row=CELLs[0]+1
        clm=CELLs[1]
        name = link+thisWeek+'\\'+date+"_morning"
